pipeline {
  ...
  stages {
    ...
    stage('SonarQube Analysis') {
      steps {
         sh 'sonar-scanner -Dsonar.host.url=http://172.31.14.53:9000 -Dsonar.login=${SONAR_USR} -Dsonar.password=${SONAR_PSW} -Dsonar.projectKey=ticketing -Dsonar.qualitygate.wait=false -Dsonar.java.binaries=./target'
      }
    }
